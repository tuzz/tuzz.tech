...
I use `zz` every day to carry out routine tasks like provisioning my machine
and generating project templates. The animation above shows some example use
cases of Bash completion, e.g. completing `zz pr` to `zz provision`.
...
Bash sometimes tries to be helpful. For example, if all available completions
start with the same prefix, Bash replaces the word at the end of the
command-line with that prefix. Usually this is helpful, but it can be
surprising.
